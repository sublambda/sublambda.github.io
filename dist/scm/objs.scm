...
(define (stringify-pretty string)
...
(define (vbox #!optional model)
  (let* ((vb (vidbox "https://rx.0x.no/stuff/td.mp4"))
         (vid (slot vb vid:)))
    (slot! vb model: model)
    (set-timeout (lambda ()
                   (send vid play:)
                   (slot! vid currentTime: 8)
                   (send vid pause:))
                 1000)
    vb))
(extern "vbox" vbox)
...
(define blockhead
  (lambda (#!optional model) (sogltf "blockhead")))
...
(define dome (lambda (#!optional model) (sogltf "dome")))
(extern "dome" dome)

(extern "lowalk" lowalk)
(extern "bash" bash)
(extern "blockhead" blockhead)
...
(define (kball #!optional model)
  (let* ((geom (%new THREE.SphereGeometry 1 12 12))
    ...
    ;;(send (slot lim csm:) setupMaterial: mat)
...
(define (ico #!optional model)
  (let* ((geom (%new THREE.IcosahedronGeometry 1 10))
         (mat (%new THREE.MeshStandardMaterial
                    (alist->object '((color: #x778888)
                                     (wireframe: #t)))))
         (mesh (%new THREE.Mesh geom mat)))
    (when (truthy? (slot lim csm:))
      (send (slot lim csm:) setupMaterial: mat))
    (slot! mesh receiveShadow: #t)
    (slot! mesh castShadow: #t)
    mesh))
...
(dynamic-import "/rasterizeHTML.min.js")
...
(define (statusbox #!optional model)
  (let* ((hb (htmlbox))
         (setstatus
          (lambda ()
            (let ((info (slot (slot lim renderer:) info:)))
              (send hb sethtml:
                    (string-append
                     ...
                     (stringify-pretty
                      (alist->object
                       `(;;("ts" ,(now))
                         ("render" ,(slot info render:))
                         ("mem" ,(slot info memory:)))))))))))
    (setstatus)
    (thread (lambda ()
              (let loop ()
                (thread-sleep! .5)
...
;;(##inline-host-declaration "import * as XCSG from 'three-bvh-csg'")
;;(##inline-host-statement "window.XCSG=XCSG")
;;\ window.XCSG=XCSG
...
(define (csg-diffop geom #!optional material)
  (csg-operation geom (slot CSG DIFFERENCE:) material))
...
(define (standard-material opts)
  (%new THREE.MeshStandardMaterial
        (alist->object (append opts '((transparent #t)
                                      (opacity: .5)
                                      (side: 2))))))
(define (grid-material opts)
  (%new CSG.GridMaterial
        (alist->object (append opts '((transparent #f)
                                      (opacity: .5)
                                      (side: 2))))))
...
(define (chunk #!optional model)
  (let* ((bg (box-geom 2 2 2))
         (bg2 (box-geom 2 2 2))
         (mat (grid-material '((color: #xffffff))))
         (mat2 (grid-material '((color: #xffbbbb))))
         (xg (let ((root (csg-addop bg (matcap0))))
               (send bg2 translate: 1 1 1)
               (send root add: (csg-subop bg2 mat))
               (send bg2 rotateX: .5)
...
(define (chunk11 #!optional model)
  (let* ((bg (box-geom 2 2 2))
         (smat (grid-material '((color #xffcccc))))
         (xg (let ((root (csg-addop bg (matcap0)))
                   (r (lambda () (* (- (random-real) .5) 2))))
               (let loop ((n 40))
                 (let ((s (sphere-geom .3)))
                   (send root add: (csg-subop s smat))
                   (send s translate: (r) (r) (r)))
                 (when (> n 0)
                   (loop (- n 1))))
               (csg-evaluate-hier root))))
    (slot! xg castShadow: #t)
...
(define (chunk1 #!optional model)
  (let* ((bg (box-geom 2 2 2))
         (mat (grid-material '((color #xccffff))))
         (smat (matcap0))
         (xg (let ((root (csg-addop bg mat))
                   (r (lambda () (* (- (random-real) .5) 2.2))))
               (send root add:
                     (csg-operation-group
                      (map (lambda (i)
                             (let ((s (sphere-geom .4)))
                               (send s translate: (r) (r) (+ .1 (r)))
                               (csg-subop s smat)))
                           (iota 8))))
               (csg-evaluate-hier root))))
    (slot! xg castShadow: #t)
...
(define (chunk3 #!optional model)
  (let* ((bg (box-geom 8 3 3))
         ...
         (xg (let ((root (csg-addop bg mat)))
               (send root add:
                     (csg-operation-group
                      (list (csg-subop eg2 mat2) (csg-subop eg mat2))))
               (csg-evaluate-hier root))))
    (slot! xg castShadow: #t)
    (slot! xg receiveShadow: #t)
    xg))
...
(extern "chunk" chunk)
(extern "chunk1" chunk1)
(extern "chunk2" chunk2)

(define (setpoints val)
  (send lim setpoints: val)) 
(extern "setpoints" setpoints)

;;(hi "objs revision: 249")
